...
def run_test():
    ...
    el_input = chiller.calc_elec_power_input(thermal_demand)
    ...
    (th_power_out, el_power_in) = chiller.calc_kkm_all_results(
        thermal_demand,
        time_index=0,
        save_res=True)
...
def run_chiller_example(print_results=False):
    ...
    for i in range(len(time_array)):
        ...
        cop_array[i] = \
            chiller.kkm_cop(curr_load)
        el_in_array[i] = \
            chiller.calc_elec_power_input(curr_load)
    ...
    chiller_op_fac = q_power_output * days_a_year * 24 / (
