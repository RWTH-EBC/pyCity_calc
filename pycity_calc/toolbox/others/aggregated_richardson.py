...
# coding=utf-8
"""
Generate aggregated richardson profiles
"""
...
def gen_aggr_rich_profiles():
    ...
    #  Sample list of occupants
    list_occ = usunc.calc_sampling_occ_per_app(nb_samples=nb_loops)

    stoch_load = np.zeros(len(time_array))

    for i in range(nb_loops):

        print('Counter: ', i+1)

        # nb_occ = random.randint(1, 5)
        nb_occ = list_occ[i]

        print('Nb. of occupants:', str(nb_occ))
        print()

        # light_conf_nb = random.randint(0, 9)

        occupancy = occ.Occupancy(environment=environment, number_occupants=nb_occ)
    ...
    out_folder = 'slp_and_aggr_rich_' + str(nb_loops) + '_no_heat'

    this_path = os.path.dirname(os.path.abspath(__file__))
    save_path = os.path.join(this_path, 'output', out_folder,
                             'aggr_stoch_load_no_heat_dev.pkl')

    pickle.dump(stoch_load, open(save_path, mode='wb'))
    ...
    #  Rescaling
    slp_rescale = 10 * 3600 * 1000 / (sum(slp_load) * timestep)
    slp_load *= slp_rescale

    stoch_rescale = 10 * 3600 * 1000 / (sum(stoch_load) * timestep)
    stoch_load *= stoch_rescale
